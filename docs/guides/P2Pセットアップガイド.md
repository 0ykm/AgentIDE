...
複数のPC上でDeck IDEを起動し、1つのブラウザUIから全ノードのターミナル・ファイル・Git・Agentを操作するための手順。
...
> **注意（Windows）**: ビルド中に `winCodeSign` のシンボリックリンクエラーが表示されることがあるが、実行ファイル（`Deck IDE.exe`）は正常に生成される。エラーメッセージは無視してよい。
...
| 接続テストが失敗する | リモートPCでDeck IDEが起動していない。サービスの起動を確認 |
